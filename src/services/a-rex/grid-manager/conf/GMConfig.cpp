...
bool GMConfig::Load() {
  ...
  return CoreConfig::ParseConf(*this) & ValidateSSH();
}
...
bool GMConfig::ValidateSSH() {
  ssh_config.validated = true;
  if (ssh_config.remote_host.empty()) return true; // OK, host is local
  logger.msg(Arc::DEBUG, "Found remote_host (%s) in arc.conf", ssh_config.remote_host);
  bool user_is_set(!ssh_config.remote_user.empty()),
       sessiondir_is_set(!ssh_config.remote_session_dir.empty()),
       pkey_is_set(!ssh_config.private_key.empty());
  if (user_is_set & sessiondir_is_set & pkey_is_set) {
    ssh_config.ok = true;
    return true; // OK, host is remote and config is valid
  }
  if (!user_is_set)
    logger.msg(Arc::ERROR, "remote_user not set in arc.conf");
  if (!sessiondir_is_set)
    logger.msg(Arc::ERROR, "remote_session_dir not set in arc.conf");
  if (!pkey_is_set)
    logger.msg(Arc::ERROR, "private_key not set in arc.conf");
  return false; // Remote host set, but insufficient config
}
