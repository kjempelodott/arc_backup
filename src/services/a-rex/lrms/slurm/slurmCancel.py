...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        raise ArcError('Usage: %s --config <arc.conf> <grami>' % sys.argv[0], 'slurmCancel')

    if sys.argv[1] != '--config':
        raise ArcError('First argument must be \'--config\' followed by path to arc.conf', 'slurmCancel')
    ...
    try:
        ...
        slurm.Cancel(arc_conf, grami.jobid)
    except Exception:
