...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        raise ArcError('Usage: %s --config <arc.conf> <grami>' % sys.argv[0], 'lsfSubmitter')
    
    if sys.argv[1] != '--config':
        raise ArcError('First argument must be \'--config\' followed by path to arc.conf', 'lsfSubmitter')
    ...
    try: 
        with open(grami, 'r') as jobdesc_file:
            ...
            if not is_parsed or not jobdescs:
                ...
                if not is_parsed or not jobdescs:
                    raise ArcError('Unable to parse job description from file (%s)' % grami, 'lsfSubmitter')
                jobdescs[0].OtherAttributes['joboption;gridid'] = str(time.time())
    except IOError:
        raise ArcError('File (%s) does not appear to exist' % grami, 'lsfSubmitter')
    
    try:
        jc = arc.compute.JobContainer()
        if lsf.Submit(arc_conf, jobdescs, jc):
            if grami[-6:] == '.grami':
                with open(grami, 'a') as fgrami:
                    fgrami.write('joboption_jobid=%s\n' % (jc[0].IDFromEndpoint))
    except Exception:
        raise ArcError('Unexpected exception:\n%s' % traceback.format_exc(), 'lsfSubmitter')    
